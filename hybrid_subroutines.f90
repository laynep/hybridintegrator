...
!!Sets the time step when calculating trajectories.
!!subroutine traj_dt(ytraj,tout,dt_traj,dt,Y,ii,iend)
!subroutine traj_dt(ytraj,tout,dt_traj,dt,Y)
!  use linked_list
!  implicit none
!
!  type(linkedlist), intent(in) :: ytraj
!  real(dp), intent(in) :: dt
!  real(dp), intent(inout) :: dt_traj, tout
!  real(dp), dimension(5), intent(in) :: Y
!  !integer, intent(in) :: ii,iend
!  real(dp), dimension(4) :: dy, dy_targ, tol
!  integer :: i
!  real(dp) :: scal
!
!  !Set target change in Y and how much tolerance to allow.
!  dy_targ = .05e0_dp
!  tol = .01e0_dp
!
!  !Calc the change in Y.
!  do i=2,5
!    dy(i-1)=abs((y(i)-ytraj%tail%a(i))/y(i))
!  end do
!
!  scal=1e0_dp
!  if (any(dy > (dy_targ + tol))) then
!    !Readjust dt_traj down.
!    !print*,"big"
!    scal=(1+abs(maxval(dy)-minval(dy_targ)))**(-1)
!    if (scal>1e0_dp) then
!      print*,"Trying to scale up when already too big..."
!      stop
!    end if
!  else if (all(dy < (dy_targ-tol))) then
!    !Readjust dt_traj up.
!    print*,"small"
!    scal=abs(log(1- (minval(dy_targ)-maxval(dy))))**(-1)
!    if (scal<1e0_dp) then
!      print*,"Trying to scale down when already too small..."
!      stop
!    end if
!  end if
!
!  !print*,"scale",scal
!
!  dt_traj=dt_traj*scal
!  if (dt_traj>dt) then
!    dt_traj=dt/1000e0_dp
!  else if (dt_traj<1e-7_dp) then
!    dt_traj=1e-7_dp
!  end if
!
!  print*,"stepsize",dt_traj
!
!  tout=tout+dt_traj
!
!!  if ((abs(Y(4)/phi_dot_0) < 1e-5_dp .and. abs(Y(5)/psi_dot_0) < 1e-5_dp) .or. &
!!    & i>iend-1000) then
!!    !small velocity
!!    !print*,"small"
!!    tout = tout + dt
!!  else if ((abs(Y(4)/phi_dot_0) < 1e-4_dp .and. abs(Y(5)/psi_dot_0) < 1e-4_dp) .or. &
!!    & i> iend*(2e0_dp/3e0_dp)) then
!!    !medium velocity
!!print*,"med"
!!    tout = tout + dt/10e0_dp
!!  else if ((abs(Y(4)/phi_dot_0) < 1e-3_dp .and. abs(Y(5)/psi_dot_0) < 1e-3_dp) .or. &
!!    & i> iend*(1e0_dp/2e0_dp)) then
!!    !medium-big velocity.
!!print*,"med-small"
!!    tout = tout + dt/1000e0_dp
!!  else if ((abs(Y(4)/phi_dot_0) < 1e-2_dp .and. abs(Y(5)/psi_dot_0) < 1e-2_dp) .or. &
!!    & i> iend*(1e0_dp/3e0_dp)) then
!!    !medium-big velocity.
!!print*,"med-med"
!!    tout = tout + dt_traj*100000e0_dp
!!  else if ((abs(Y(4)/phi_dot_0) < 1e-1_dp .and. abs(Y(5)/psi_dot_0) < 1e-1_dp) .or. &
!!    & i> iend*(1e0_dp/4e0_dp)) then
!!    !medium-big velocity.
!!print*,"med-big"
!!    tout = tout + dt_traj*1000e0_dp
!!  else
!!    !big velocity
!!!print*,"big"
!!    tout = tout + dt_traj
